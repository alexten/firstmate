...
| `fm-q-guard-lib.sh`      | Obtain, commit, and release durable Quartermaster child-spawn leases, including one selected opaque no-mistakes custodian |
| `fm-api.sh`              | Expose Q-managed worker lifecycle owners and the additive `fm.opaque-no-mistakes.v1` compatibility result through one versioned JSON facade |
